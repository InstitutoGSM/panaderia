...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($accion) {
        ...
        case 'editar_panaderia':
            ...
            if (!$nombre || !$email) { echo json_encode(['ok'=>false,'msg'=>'Nombre y email son obligatorios']); exit; }
        ...
        case 'cambiar_pass_panaderia':
            ...
            if (strlen($pass) < 6) { echo json_encode(['ok'=>false,'msg'=>'Mínimo 6 caracteres']); exit; }
...
<head>
  ...
  <style>
    /* ── Tabs ─────────────────────────────────────────────────────── */
    .admin-tabs { display:flex; gap:8px; margin-bottom:28px; border-bottom:2px solid var(--crema-dark); padding-bottom:0; }
    .admin-tab  { padding:10px 22px; font-weight:700; font-size:0.92rem; cursor:pointer;
                  border:none; background:none; color:var(--gris); border-radius:8px 8px 0 0;
                  border-bottom:3px solid transparent; transition:all .2s; }
    .admin-tab:hover  { color:var(--marron); background:var(--crema-dark); }
    .admin-tab.activo { color:var(--marron); border-bottom-color:var(--naranja); background:var(--blanco); }
    .tab-panel { display:none; }
    .tab-panel.activo { display:block; }

    /* ── Tabla usuarios ───────────────────────────────────────────── */
    .tabla-admin { width:100%; border-collapse:collapse; background:var(--blanco);
                   border-radius:var(--radio-lg); overflow:hidden; box-shadow:var(--sombra); }
    .tabla-admin th { background:var(--marron); color:white; padding:12px 16px;
                      text-align:left; font-size:0.82rem; font-weight:700; }
    .tabla-admin td { padding:12px 16px; border-bottom:1px solid var(--crema-dark);
                      font-size:0.88rem; vertical-align:middle; }
    .tabla-admin tr:last-child td { border-bottom:none; }
    .tabla-admin tr:hover td { background:var(--crema); }

    /* ── Cards panaderias ─────────────────────────────────────────── */
    .pan-card { background:var(--blanco); border-radius:var(--radio-lg);
                box-shadow:var(--sombra); margin-bottom:16px; overflow:hidden; }
    .pan-card-head { display:flex; align-items:center; gap:14px;
                     padding:16px 20px; cursor:pointer; }
    .pan-card-head:hover { background:var(--crema); }
    .pan-info { flex:1; }
    .pan-info h3 { font-family:'Playfair Display',serif; font-size:1rem; margin:0 0 2px; }
    .pan-info p  { font-size:0.8rem; color:var(--gris); margin:0; }
    .pan-body { display:none; border-top:1px solid var(--crema-dark); padding:16px 20px; }
    .pan-body.open { display:block; }
    .pan-acciones { display:flex; gap:8px; flex-wrap:wrap; margin-top:12px; }

    /* ── Toggle admin ─────────────────────────────────────────────── */
    .toggle-wrap { display:flex; align-items:center; gap:10px; background:var(--crema);
                   border-radius:50px; padding:8px 14px; }
    .toggle-switch { position:relative; width:44px; height:24px; display:inline-block; }
    .toggle-switch input { opacity:0; width:0; height:0; }
    .toggle-slider { position:absolute; inset:0; background:#ccc; border-radius:50px;
                     cursor:pointer; transition:.3s; }
    .toggle-slider:before { content:''; position:absolute; width:18px; height:18px;
                             left:3px; bottom:3px; background:white; border-radius:50%;
                             transition:.3s; }
    input:checked + .toggle-slider { background:var(--naranja); }
    input:checked + .toggle-slider:before { transform:translateX(20px); }

    /* ── Sucursales ───────────────────────────────────────────────── */
    .suc-list { margin-top:10px; }
    .suc-item { display:flex; align-items:center; gap:8px; padding:6px 10px;
                background:var(--crema); border-radius:8px; margin-bottom:6px;
                font-size:0.84rem; }
    .suc-item-vacio { font-size:0.82rem; color:var(--gris); font-style:italic; }

    /* ── Modales ──────────────────────────────────────────────────── */
    .modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,.5);
                     z-index:500; display:flex; align-items:center;
                     justify-content:center; padding:20px; }
    .modal-box { background:var(--blanco); border-radius:var(--radio-lg);
                 padding:28px; width:100%; max-width:480px; box-shadow:var(--sombra-lg); }
    .modal-box h3 { font-family:'Playfair Display',serif; margin-bottom:16px; }
    .modal-campo { margin-bottom:14px; }
    .modal-campo label { display:block; font-size:0.82rem; font-weight:700;
                         color:var(--marron); margin-bottom:4px; }
    .modal-campo input { width:100%; box-sizing:border-box; }
    .modal-acciones { display:flex; gap:10px; margin-top:18px; }
    .badge-tipo { display:inline-block; padding:2px 10px; border-radius:50px;
                  font-size:0.75rem; font-weight:700; }
    .badge-vendedor { background:#FFF3E0; color:#E65100; }
    .badge-comprador { background:#E3F2FD; color:#1565C0; }
  </style>
</head>
<body>
          ...
          <div class="pan-body">
            ...
            <div class="pan-acciones">
              <button class="btn btn-sm"
                onclick="abrirEditarPan(<?= $v['id'] ?>, '<?= addslashes(h($v['nombre'])) ?>', '<?= addslashes(h($v['nombre_panaderia'] ?? '')) ?>', '<?= addslashes(h($v['email'])) ?>', '<?= addslashes(h($v['email_contacto'] ?? '')) ?>')">
                ✏️ Editar info
              </button>
              <button class="btn btn-sm btn-ghost"
                onclick="abrirCambiarPass(<?= $v['id'] ?>, '<?= addslashes(h($v['nombre_panaderia'] ?: $v['nombre'])) ?>')">
                🔑 Cambiar contraseña
              </button>
            </div>
          </div>
          ...
          <tr data-nombre="<?= strtolower(h($u['nombre'])) ?>" data-email="<?= strtolower(h($u['email'])) ?>">
            ...
            <td>
              <button class="btn btn-sm" style="margin-right:6px"
                onclick="abrirEditarUsuario(<?= $u['id'] ?>, '<?= addslashes(h($u['nombre'])) ?>', '<?= addslashes(h($u['email'])) ?>')">
                ✏️ Editar
              </button>
              <button class="btn btn-sm btn-ghost"
                onclick="abrirResetPass(<?= $u['id'] ?>, '<?= addslashes(h($u['nombre'])) ?>')">
                🔑 Resetear pass
              </button>
            </td>
          </tr>
...
async function toggleAdmin(uid, habilitado) {
  const ok = await postAdmin({ accion:'toggle_admin', uid, valor: habilitado ? 1 : 0 });
  if (!ok) {
    // Revertir si falló
...
async function guardarEditarPan() {
  ...
  if (ok) { cerrarModal('modal-editar-pan'); setTimeout(()=>location.reload(),1200); }
}
...
async function guardarEditarUsuario() {
  ...
  if (ok) { cerrarModal('modal-editar-usuario'); setTimeout(()=>location.reload(),1200); }
}
...
async function guardarCorreccion() {
  ...
  if (ok) { cerrarModal('modal-correccion'); accionVendedor._updateCard('corregir', parseInt(document.getElementById('corr-vid').value)); }
}

/* ══ VERIFICACION ═══════════════════════════════════════════════ */
async function accionVendedor(accion, vid) {
  if (accion === 'corregir') { abrirCorreccion(vid); return; }
  if (accion === 'rechazar' && !confirm('¿Rechazar este vendedor?')) return;

  const ok = await postAdmin({ accion, vendedor_id: vid });
  if (ok) {
...
function actualizarStats() {
  ...
  cards.forEach(card => { const e=card.dataset.estado; if(c[e]!==undefined) c[e]++; });
