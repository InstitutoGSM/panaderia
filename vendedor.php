...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'add_producto') {
        ...
        if (!$nombre || $precio <= 0) {
            ...
            if (!empty($_FILES['imagen']['name'])) {
                ...
                if (!$img_url) { $msg_err = 'Error al subir imagen (máx 5MB, jpg/png/webp).'; $seccion = 'add'; }
            }
            if (!$msg_err) {
                db()->prepare("
    ...
    if ($accion === 'edit_producto') {
        ...
        $chk->execute([$pid, $uid]);
        if (!$chk->fetch()) {
            $msg_err = 'Producto no encontrado.'; $seccion = 'productos';
        } elseif (!$nombre || $precio <= 0) {
            $msg_err = 'Completá nombre y precio.'; $seccion = 'add';
        } else {
            $img_url = null;
    ...
    if ($accion === 'toggle') {
        ...
        header('Location: vendedor.php?sec=productos'); exit;
    }
    ...
    if ($accion === 'delete') {
        ...
        header('Location: vendedor.php?sec=productos&ok=eliminado'); exit;
    }
    ...
    if ($accion === 'estado_pedido') {
        ...
        header('Location: vendedor.php?sec=pedidos'); exit;
    }
    ...
    if ($accion === 'perfil') {
        ...
        if (in_array('transferencia', $medios) && !$cbu && !$alias) {
            ...
            db()->prepare("UPDATE usuarios SET
              ...
              WHERE id=?")
            ->execute([$nombre,$nombre_pan,$desc,$banner,$instagram,$telefono,$email_c,
                       $cbu,$alias,$titular,$medios_str,$avatar_url,$uid]);
            $u       = usuario_actual();
    ...
    <div class="sec-card">
      ...
      <?php
        $doc_lista = [
          ...
          ['col'=>'doc_carnet_manipulador',     'n'=>2,'ico'=>'🪪',
          ...
          ['col'=>'doc_habilitacion_comercial', 'n'=>3,'ico'=>'🏪',
...
<?php if ($msg_ok): ?>toast('<?= addslashes($msg_ok) ?>', 'ok');<?php endif; ?>
</script>
